...
def calculate_advanced_trend_strength_with_wavelets(dataframe: pd.DataFrame, strong_threshold: float = 0.02, pair: str = None, feature_cache: dict = None, last_cache_update: dict = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        cache_key = f"{pair}_fft_{len(dataframe)}" if pair else f"default_fft_{len(dataframe)}"
        ...
        if (use_cache and 
            cache_key in feature_cache and 
            cache_key in last_cache_update and
            (current_time - last_cache_update[cache_key]).seconds < 300):  # 5 min for 1h candles
            ...
            if len(dataframe) > 500:
                ...
                cycle_strength_full = pd.concat([
                    pd.Series([0] * padding_length, index=dataframe.index[:padding_length]),
                    cycle_str
                ])
...
class AdvancedPredictiveEngine:
    ...
    def __init__(self):
        ...
        if self.legacy_models_dir.exists() and list(self.legacy_models_dir.glob("*.pkl")):
            self.models_dir = self.legacy_models_dir
            logger.info("[ML-V4] Using legacy models directory: user_data/strategies/ml_models")
        else:
            self.models_dir.mkdir(parents=True, exist_ok=True)
            logger.info("[ML-V4] Using new models directory: ml_models")
        
        # V4: Async training infrastructure
        self.training_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="MLTrain")
    ...
    def _load_pair_models(self, pair: str):
        ...
        if models:
            ...
            logger.info(f"[ML-V4] Loaded {len(models)} models for {pair} (full ensemble ready)")
    ...
    def _cleanup_old_models(self, max_age_days: int = 7):
        ...
        try:
            ...
            for pair in self.models.keys():
                ...
                for pattern in [f"{safe_pair}_model_*.pkl", f"{safe_pair}_scaler.pkl", f"{safe_pair}_metadata.pkl"]:
            ...
            for model_file in self.models_dir.glob("*.pkl"):
                ...
                if is_temp and is_old:
                    ...
                    model_file.unlink()
                    logger.info(f"[ML-V4] Removed old inactive model: {model_file.name}")
                    cleaned_count += 1
    ...
    def extract_advanced_features_v3(self, df: pd.DataFrame, pair: str) -> pd.DataFrame:
        ...
        if 'btc_close' in df.columns:
            ...
            df['btc_divergence'] = (df['close'].pct_change() - df['btc_close'].pct_change()).rolling(20).mean()
        
        # 2. Advanced volatility metrics
        returns = df['close'].pct_change()
        df['garch_volatility'] = self._estimate_garch_volatility(returns)
        df['realized_volatility'] = np.sqrt((returns ** 2).rolling(20).mean()) * np.sqrt(24)
        ...
        for period in [10, 20, 50]:
            df[f'ker_{period}'] = self._calculate_kaufman_efficiency_ratio(df['close'], period)
        
        # B. Bollinger Bandwidth and Squeeze
        bb_period = 20
        bb_std = 2
        bb_upper = df['close'].rolling(bb_period).mean() + df['close'].rolling(bb_period).std() * bb_std
        bb_lower = df['close'].rolling(bb_period).mean() - df['close'].rolling(bb_period).std() * bb_std
        df['bb_bandwidth'] = (bb_upper - bb_lower) / df['close']
        df['bb_bandwidth_z'] = (df['bb_bandwidth'] - df['bb_bandwidth'].rolling(50).mean()) / (df['bb_bandwidth'].rolling(50).std() + 1e-10)
        ...
        returns = df['close'].pct_change()
        for lag in [1, 2, 3]:
            df[f'autocorr_ret_lag{lag}'] = returns.rolling(20).apply(lambda x: x.autocorr(lag=lag) if len(x) > lag else 0)
        ...
        if 'rsi_divergence_bull' in df.columns:
            ...
            df['bars_since_bull_div'] = (df['rsi_divergence_bull'] == 1).astype(int).groupby((df['rsi_divergence_bull'] == 1).cumsum()).cumcount()
        
        # F. Breakout pressure
        df['closes_above_high20'] = (df['close'] > df['high'].shift(1).rolling(20).max()).rolling(20).mean()
        ...
        if 'eth_close' in df.columns:
            ...
            df['eth_divergence'] = (df['close'].pct_change() - df['eth_close'].pct_change()).rolling(20).mean()
        
        # H. Underwater metrics (drawdown)
        rolling_max = df['close'].expanding().max()
        df['drawdown_pct'] = (df['close'] - rolling_max) / rolling_max
        df['bars_in_drawdown'] = (df['close'] < rolling_max).astype(int).groupby((df['close'] >= rolling_max).cumsum()).cumcount()
        ...
        if 'close_4h' in df.columns:
            ...
            df['vol_realized_4h'] = np.sqrt((df['ret_4h'] ** 2).rolling(20).mean()) * np.sqrt(6)  # 6 = 24h/4h
            ...
            if 'ker_20' in df.columns:
                df['ker_4h'] = self._calculate_kaufman_efficiency_ratio(df['close_4h'], 20).shift(1)
        
        # Check for 15m data  
        if 'close_15m' in df.columns:
            df['ret_15m'] = df['close_15m'].pct_change()
            df['momentum_15m'] = df['close_15m'].pct_change(4)  # 1h momentum in 15m bars
            # Add lag to prevent lookahead
    ...
    def extract_advanced_features(self, dataframe: pd.DataFrame, lookback: int = 100, pair: str = None) -> pd.DataFrame:
        ...
        if 'maxima_sort_threshold' in df.columns:
            ...
            df['resistance_strength'] = 0.5  # Neutral value when thresholds not available
    ...
    def _calculate_beta(self, asset_returns: pd.Series, market_returns: pd.Series, window: int) -> pd.Series:
    ...
    def _estimate_garch_volatility(self, returns: pd.Series) -> pd.Series:
        ...
        for i in range(21, len(returns)):
            garch_vol.iloc[i] = np.sqrt(
                omega + alpha * returns.iloc[i-1]**2 + beta * garch_vol.iloc[i-1]**2
            )
        return garch_vol
            ...
            if p_window.notna().sum() > 10 and v_window.notna().sum() > 10:
                ...
                for p_val in p_marginal.index:
                    for v_val in v_marginal.index:
                        if p_val in joint_counts.index and v_val in joint_counts.columns:
                            joint_prob = joint_counts.loc[p_val, v_val]
                            if joint_prob > 0:
                                mi += joint_prob * np.log2(joint_prob / (p_marginal[p_val] * v_marginal[v_val] + 1e-10))
                mi_series.iloc[i] = mi
    ...
    def _calculate_prediction_confidence(self, df: pd.DataFrame, pair: str) -> pd.Series:
    ...
    def _calculate_kaufman_efficiency_ratio(self, series: pd.Series, period: int) -> pd.Series:
    ...
    def train_predictive_models_async(self, df: pd.DataFrame, pair: str) -> None:
        ...
        # Submit to executor
        future = self.training_executor.submit(self._train_models_worker, df.copy(), pair)
        ...
        def on_complete(fut):
            try:
                ...
                logger.info(f"[ML-V4] Async training completed for {pair} in {elapsed:.1f}s")
                if result.get('status') == 'success':
                    # Atomic model swap happens inside _train_models_worker
                    self.model_versions[pair] = self.model_versions.get(pair, 0) + 1
                    logger.info(f"[ML-V4] {pair} promoted to model v{self.model_versions[pair]}")
            except Exception as e:
    ...
    def train_predictive_models(self, df: pd.DataFrame, pair: str, output_dir: Optional[Path] = None) -> dict:
        ...
        try:
            ...
            market_volatility = base_df['close'].pct_change().rolling(20).std().iloc[-1] if len(base_df) > 20 else 0.02
            ...
            target = self.create_target_variable(base_df, 
                                                forward_periods=adaptive_horizon,
                                                dynamic=dynamic)
            ...
            logger.info(f"[ML-V4] Starting Walk-Forward Validation with {n_wfv_splits} windows, gap={purge_gap}")
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(wfv_splitter.split(X)):
                ...
                logger.info(f"[ML-V4] WFV Fold {fold_idx+1}/{n_wfv_splits}: "
            ...
            if wfv_results['f1_scores']:
                ...
                logger.info(f"[ML-V4] {pair} WFV Summary: F1={wfv_f1_mean:.3f}±{wfv_f1_std:.3f}, "
            ...
            logger.info(f"Final split - Train: {len(X_train)}, Test: {len(X_test)}, Gap: {purge_gap}")
            ...
            logger.info(f"[ML-V4] Using {calibration_method} calibration for {pair} (n={len(X_train)})")
            ...
            from sklearn.calibration import CalibratedClassifierCV
            rf_calibrated = CalibratedClassifierCV(rf_grid.best_estimator_, method=calibration_method, cv=3)
            ...
            # V4: Calibrate probability with adaptive method
            gb_calibrated = CalibratedClassifierCV(gb_grid.best_estimator_, method=calibration_method, cv=3)
            ...
            # V4: Calibrate probability with adaptive method
            ada_calibrated = CalibratedClassifierCV(ada, method=calibration_method, cv=3)
            ...
            if len(X_train) < 2000:  # Only for smaller datasets due to computational cost
                ...
                svm_calibrated = CalibratedClassifierCV(svm, method=calibration_method, cv=3)
            ...
            X_train_hgb = X_train.values if isinstance(X_train, pd.DataFrame) else X_train
            hist_gb.fit(X_train_hgb, y_train)  # HGB handles its own scaling
            # V4: Calibrate probability with adaptive method
            hist_gb_calibrated = CalibratedClassifierCV(hist_gb, method=calibration_method, cv=3)
            hist_gb_calibrated.fit(X_train_hgb, y_train)  # Use same format for consistency
            ...
            logger.info(f"{pair} HistGradientBoosting: Acc={hist_accuracy:.3f}, F1={hist_f1:.3f}")
            ...
            for name, model in models.items():
                ...
                cv_scores = cross_val_score(model, X_train_scaled, y_train, cv=purged_cv, scoring='f1')
            ...
            if len(results) >= 3:
                ...
                logger.info(f"{pair} Stacking Ensemble: Acc={stacking_accuracy:.3f}, F1={stacking_f1:.3f}")
                ...
                if stacking_f1 > ensemble_f1:
                    ...
                    logger.info(f"{pair} Stacking outperformed Voting (F1: {stacking_f1:.3f} > {ensemble_f1:.3f})")
    ...
    def predict_entry_probability(self, df: pd.DataFrame, pair: str) -> pd.Series:
        ...
        try:
            ...
            if hasattr(self, 'model_metadata') and pair in self.model_metadata:
                ...
                logger.debug(f"[ML-V4] Using {len(feature_columns)} canonical features for {pair}")
            elif pair in self.feature_importance and 'random_forest' in self.feature_importance[pair]:
...
def calculate_advanced_predictive_signals(dataframe: pd.DataFrame, pair: str, base_threshold: float = 0.02) -> pd.DataFrame:
    ...
    try:
        ...
        if not need_training and 'ml_entry_probability' in dataframe.columns and len(dataframe) > 500:
            ...
            historical_preds = dataframe['ml_entry_probability'].iloc[-500:-100].dropna()
            ...
            if len(recent_preds) > 50 and len(historical_preds) > 50:
                ...
                if ks_stat > 0.3 and p_value < 0.05:
                    need_training = True
                    logger.info(f"[ML-V4] Drift detected for {pair} (KS={ks_stat:.3f}, p={p_value:.4f})")

        if need_training:
            logger.info(f"[ML-V4] Triggering async training for {pair} (len={len(dataframe)}) | assets_exist={assets_exist}")
        ...
        h = getattr(predictive_engine, "prediction_horizon", 5)  # Usar mismo horizonte que el modelo
        ...
        dataframe['pred_q80'] = dataframe['ml_entry_probability'].rolling(window, min_periods=50).quantile(0.80)
        ...
        if 'atr' in dataframe.columns:
            rel_atr = (dataframe['atr'] / dataframe['close']).rolling(20, min_periods=5).mean()
            vol_factor = float((1.0 + rel_atr.fillna(0.01) * 10.0).clip(1.0, 1.5).iloc[-1])
        else:
            vol_factor = 1.0
        ...
        if pair and random.random() < 0.25:  # 25% del tiempo para mejor visibilidad
            logger.info(f"[ADAPTIVE] {pair} model_temp={model_temp:.3f} vol_factor={vol_factor:.2f} "
...
class AlexNexusForgeV8AIV4(IStrategy):
    ...
    process_only_new_candles = True  # More efficient in live trading, avoids intra-candle re-evaluations
    ...
    trailing_exit_min_profit = DecimalParameter(-0.03, 0.02, default=0.0, decimals=3,
                                               space="sell", optimize=True, load=True)
    # 3. REMOVIDO timed_exit_hours - usando ROI table para salidas temporales (24h/48h)
    strong_threshold = DecimalParameter(0.005, 0.08, default=0.020, decimals=3, space="buy", optimize=True, load=True)
    ...
    atr_stoploss_multiplier = DecimalParameter(0.8, 2.0, default=1.2, decimals=1, space="sell", optimize=True, load=True)
    ...
    atr_stoploss_ceiling = DecimalParameter(-0.10, -0.06, default=-0.08, decimals=2, space="sell", optimize=True, load=True)
    # DCA parameters
    ...
    cooldown_lookback = IntParameter(2, 48, default=3, space="protection", optimize=True)  # Subido a 3 velas
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if current_profit > 0.01:  # Start trailing at 1% profit instead of 3%
            ...
            if trailing_offset > 0:
                calculated_stoploss = max(calculated_stoploss, -trailing_offset)  # Trail up in profit correctly
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        try:
            ...
            if metadata['pair'] != 'ETH/USDT':
                ...
                if not eth_info.empty and len(eth_info) >= len(dataframe):
                    eth_close_data = eth_info['close'].tail(len(dataframe)).reset_index(drop=True)
                    dataframe['eth_close'] = eth_close_data.values
                    logger.info(f"{metadata['pair']} ETH correlation data added successfully")
                else:
                    dataframe['eth_close'] = dataframe['close']  # fallback
                    logger.warning(f"{metadata['pair']} ETH data unavailable, using pair data as fallback")
            else:
        ...
        try:
            ...
            if not inf_4h.empty:
                ...
                inf_4h['atr_4h'] = ta.ATR(inf_4h['high'], inf_4h['low'], inf_4h['close'], timeperiod=14)
            ...
            if not inf_8h.empty:
                ...
                inf_8h['atr_8h'] = ta.ATR(inf_8h['high'], inf_8h['low'], inf_8h['close'], timeperiod=14)
            ...
            if metadata['pair'] != 'BTC/USDT':
                ...
                if not btc_4h.empty:
                    ...
                    btc_4h_clean = btc_4h_clean.rename(columns={
                        'close': 'btc_close',
                        'high': 'btc_high', 
                        'low': 'btc_low'
                    })
                    # Calculate indicators using renamed columns
                    btc_4h_clean['btc_rsi_4h'] = ta.RSI(btc_4h_clean['btc_close'], timeperiod=14)
                    btc_4h_clean['btc_ema50_4h'] = ta.EMA(btc_4h_clean['btc_close'], timeperiod=50)
                    # Merge with proper suffix handling
                    dataframe = merge_informative_pair(dataframe, btc_4h_clean, self.timeframe, '4h', ffill=True)
                    
        except Exception as e:
            logger.warning(f"{metadata['pair']} Multi-timeframe integration failed: {e}")
            # Set default values for missing columns
            for col in ['rsi_4h', 'ema50_4h', 'atr_4h', 'rsi_8h', 'ema50_8h', 'atr_8h', 
                       'rsi_1d', 'ema20_1d', 'btc_rsi_4h', 'btc_ema50_4h']:
        ...
        try:
            ...
            if 'thr_dyn' in dataframe.columns and len(dataframe) > 100:
                ...
                logger.info(f"[ML-V4] {pair} Analysis completed in {t_elapsed:.1f}s | "
                ...
                logger.info(f"[ML-V4] {pair} Advanced predictive analysis completed in {t_elapsed:.1f}s")
        except Exception as e:
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        atr_ok = atr_rel < (atr_rel.rolling(200).median().fillna(atr_rel.median()) * 1.8)
        ...
        ml_agreement = dataframe.get('ml_model_agreement', pd.Series(0.5, index=dataframe.index))
        
        # 2. EXTENDER FILTRO DE RÉGIMEN al gate principal
        # Evitar whipsaws cuando RSI≥50 y no hay empuje de tendencia
        trend_strength = dataframe.get('trend_strength', pd.Series(0, index=dataframe.index))
        ...
        ml_enhanced_required[(current_rsi >= 35) & (current_rsi < 50)] = 0.12  # 35 ≤ RSI < 50 → ml_enhanced ≥ 0.12
        # RSI < 35 mantiene 0.10 por defecto
        
        # E. FILTRO ADICIONAL: trend_strength para RSI >= 50 (ya definido arriba)
        trend_filter = (current_rsi < 50) | (trend_strength > 0)  # Solo exigir trend si RSI >= 50
        ...
        if len(dataframe) > 0:
            ...
            last_ml_enhanced = ml_enhanced.iloc[-1] if isinstance(ml_enhanced, pd.Series) else -1
            last_ml_prob = ml_prob.iloc[-1] if isinstance(ml_prob, pd.Series) else -1
            last_ml_agreement = ml_agreement.iloc[-1] if isinstance(ml_agreement, pd.Series) else -1
            last_ev_filter = ev_filter.iloc[-1] if isinstance(ev_filter, pd.Series) else -1
            
            # ALWAYS log to see what's happening
            logger.info(f"OPPO_VALUES: {pair} | RSI={last_rsi:.2f} | ml_enhanced={last_ml_enhanced:.3f} | ml_prob={last_ml_prob:.3f} | ml_agreement={last_ml_agreement:.3f} | ev_filter={last_ev_filter}")
            
            # Check if opportunistic conditions are being met
            if last_rsi > 0 and last_rsi < 35:
                logger.info(f"OPPO_TRIGGER: {pair} RSI={last_rsi:.2f} < 35! Checking other conditions...")
        
        # TIER 1: Ultra AI Confidence (with dynamic threshold)
        ai_ultra_long = (
            gate &
            (ml_enhanced > 0.65) &  # Enhanced score still high
            basic_safety
        )
        ...
        ai_opportunistic_long = (
            ...
            (dataframe['volume'] > dataframe['avg_volume'] * 1.2) &  # Volumen reducido a 1.2x
            basic_safety &
        ...
        if ai_opportunistic_long.any():
            ...
            if signal_indices:
                ...
                logger.info(f"OPPO_SIGNAL: Found {ai_opportunistic_long.sum()} opportunistic entry signals!")
                logger.info(f"  Last signal at index {last_signal_idx}: RSI={dataframe.loc[last_signal_idx, 'rsi']:.2f}")
    ...
    def _populate_custom_exits_advanced(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        try:
            ...
            pre_degradation_long = (
                (ml_prob < 0.48) &  # Umbral más alto para pre-degradación
                (trend_strength < trend_strength.shift(3))  # Trend cayendo
            )
            ...
            pre_degradation_short = (
                (ml_prob > 0.52) &  # Umbral para pre-degradación short
                (trend_strength > trend_strength.shift(3))  # Trend subiendo
            )
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
                              ...
                              current_entry_rate: float, current_entry_profit: float,
                              current_exit_rate: float, current_exit_profit: float,
        ...
        try:
            ...
            if filled_entries >= self.max_safety_orders.value + 1:  # +1 for initial order
                return None
            ...
            if current_profit <= trigger:
                ...
                logger.info(f"[DCA-V4] {trade.pair} triggering safety order {filled_entries} "
