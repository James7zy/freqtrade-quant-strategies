...
class AdvancedPredictiveEngine:
    ...
    # --- GPU (AMD/OpenCL) LightGBM optional acceleration flags ---
        # Set to True to attempt LightGBM GPU training (requires LightGBM compiled with USE_GPU=1 and AMD OpenCL runtime)
        self.enable_lightgbm_gpu: bool = True
        # Detect OpenCL presence for AMD GPU (basic heuristic; will still try import later)
        self.opencl_available: bool = self._detect_opencl()
    ...
    def _required_asset_paths(self, pair: str) -> list[Path]:
        ...
        return [
            ...
            # LightGBM GPU model is optional; do not enforce for asset existence
    ...
    def _detect_opencl(self) -> bool:
        """Rudimentary OpenCL availability check for AMD GPU acceleration.

        Returns True if system appears to have OpenCL ICDs (e.g., AMD ROCm / amdgpu OpenCL)
        This is intentionally lightweight and failure-safe: any exception returns False.
        """
        try:
            import os  # noqa: WPS433
            import subprocess  # noqa: WPS433
            import shutil  # noqa: WPS433
            # Heuristic: common OpenCL/ROCm paths
            if os.path.isdir('/etc/OpenCL') or os.path.isdir('/opt/rocm'):
                return True
            # Try clinfo if present
            clinfo_bin = shutil.which('clinfo')
            if clinfo_bin:
                try:
                    out = subprocess.check_output([clinfo_bin], stderr=subprocess.STDOUT, timeout=2).decode(errors='ignore')
                    if 'Platform Name' in out or 'Number of platforms' in out:
                        return True
                except Exception:
                    pass
            # Try importing pyopencl (optional)
            try:
    ...
    def _load_pair_models(self, pair: str):
        ...
        safe_pair = pair.replace('/', '_').replace(':', '_')  # noqa: F841 retained for clarity (filename derivation)

        # Load models
        models = {}
        for model_name in ['random_forest', 'gradient_boosting', 'lightgbm_gpu']:
            ...
            if model_filepath.exists():
                with open(model_filepath, 'rb') as f:
                    try:
                        ...
                        logger.warning(f"Failed to load model {model_name} for {pair}: {e}")
    ...
    def train_predictive_models(self, df: pd.DataFrame, pair: str) -> dict:
        ...
        try:
            ...
            # === (OPTIONAL) MODEL 7: LightGBM GPU (AMD OpenCL) ===
            # Only attempted if user enabled and OpenCL detected. Fails gracefully otherwise.
            if self.enable_lightgbm_gpu and self.opencl_available:
                try:
                    import lightgbm as lgb  # LightGBM must be compiled with GPU support (OpenCL)
                    lgb_params = {
                        'objective': 'binary',
                        ...
                        'min_data_in_leaf': 25,
                        'lambda_l2': 1.0,
                        'device_type': 'gpu',  # OpenCL GPU path
                        'gpu_platform_id': 0,
                        'gpu_device_id': 0,
                        'verbosity': -1,
                    }
                    # LightGBM expects 2D arrays; scaled arrays already satisfy.
                    dtrain = lgb.Dataset(X_train_scaled, label=y_train)
                    dvalid = lgb.Dataset(X_test_scaled, label=y_test, reference=dtrain)
                    lgb_model = lgb.train(
                        lgb_params,
                        dtrain,
                        num_boost_round=400,
                    ...
                    class LGBWrapper:  # Minimal sklearn-like interface for downstream uniform handling
                        def __init__(self, booster):
                            self.booster = booster
                        def predict(self, X):  # binary class output
                    ...
                    models['lightgbm_gpu'] = LGBWrapper(lgb_model)
                    logger.info(f"{pair} LightGBM GPU model trained (best_iter={lgb_model.best_iteration})")
                except Exception as e:  # noqa: W0703 - we want to swallow all errors gracefully here
                    logger.warning(f"{pair} LightGBM GPU training failed, skipped: {e}")
